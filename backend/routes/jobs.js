...
router.post('/', async(req, res) => {
    ...
    try{
        ...
        const {data,error} = await supabase
            .from('job_application')
            .insert([{ user_id:'00000000-0000-0000-0000-000000000000', company_name, role, status, platform, notes, job_url, date_applied }]).select();
...
router.put('/:id', async(req,res) => {
    ...
    try {
        ...
        const {data, error} = await supabase
            .from('job_application')
            .update({status, notes, platform, role}).eq('job_id', jobId).select();
